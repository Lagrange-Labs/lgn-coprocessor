...
fn register_provers(config: &Config, manager: &mut ProversManager) {
    ...
    if config.worker.instance_type >= WorkerClass::Medium {
        ...
        register_v0_preprocessor(config, manager);
        info!("Preprocessing prover created");
    }

    if config.worker.instance_type >= WorkerClass::Large {
        info!("Creating groth16 prover");
        register_v0_groth16_prover(config, manager);
        info!("Groth16 prover created");
    }
}

fn register_v0_groth16_prover(config: &Config, manager: &mut ProversManager) {
    ...
    manager.add_prover(ProverType::Query2Groth16, Box::new(groth16_prover));
}

fn register_v0_preprocessor(config: &Config, manager: &mut ProversManager) {
    let params_config = &config.public_params;
...
fn register_v0_ecr20_query_prover(config: &Config, manager: &mut ProversManager) {
    ...
    manager.add_prover(ProverType::QueryErc20, Box::new(query3_prover));
}
